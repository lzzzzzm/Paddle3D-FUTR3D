...
两者在前向时，采用[detr3d_resnet101预训练模型](https://drive.google.com/file/d/1YWX-jIS6fxG5_JKUBNVcZtsPtShdjE4O/view?usp=sharing)的预训练权重，其中由于部分参数名字不一样，需要重新更改名字。
...
Paddle加载预训练权重**cam_only_pretrained.pdparams**, torch加载预训练权重**futr3d_cam_only.pth**
...
Paddle加载预训练权重**cam_only_init.pdparams**, torch加载预训练权重**[torch预训练权重](https://drive.google.com/drive/folders/1h5bDg7Oh9hKvkFL-dRhu5-ahrEp2lRNN?usp=sharing)**，其中cam_only_init为torch载入后预训练权重后，在开始阶段保存并权重转换得到。
...
<center>
    ...
    display: inline-block;
    color: #999;
    padding: 2px;">保存torch载入权重后，转换权重即可得到cam_only_init</div>
</center>

反向时，使用SGD和momentum优化器来进行，并去掉梯度裁剪等操作，如图。
...
<center>
    ...
    display: inline-block;
    color: #999;
    padding: 2px;">优化器修改</div>
</center>


## Transformer 使用6层layer
<center>
    <img style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="figure/8.png">
