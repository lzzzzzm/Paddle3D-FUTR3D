import paddle
from paddle3d.models.bbox.demodata import ensure_rng
...
class AssignResult(object):
    ...
    def random(cls, **kwargs):
        ...
        if num_gts == 0:
            max_overlaps = paddle.zeros(num_preds, dtype='float32')
            gt_inds = paddle.zeros(num_preds, dtype='float32')
            if p_use_label is True or p_use_label < rng.rand():
                labels = paddle.zeros(num_preds, dtype='int64')
            else:
                labels = None
        else:
            import numpy as np
            ...
            gt_inds = paddle.zeros(num_preds, dtype='int64')
    ...
    def add_gt_(self, gt_labels):
        ...
        self_inds = paddle.arange(
            1, len(gt_labels) + 1, dtype='long', device=gt_labels.place)
