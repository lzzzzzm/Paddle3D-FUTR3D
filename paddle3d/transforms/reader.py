...
__all__ = [
    "LoadImage", "LoadPointCloud", "LoadPointsFromFile", "LoadPointsFromMultiSweeps",
...
class LoadMultiViewImageFromFiles(TransformABC):
    ...
    def __call__(self, sample):
        ...
        for name in filename:
            name = name.replace('\\', '/')
            re_filename.append(name)
        img = np.stack(
            [cv2.imread(name, self.imread_flag) for name in re_filename], axis=-1)
        if self.to_float32:
